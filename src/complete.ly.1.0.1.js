...
function completely(container, config) {
    ...
    config.maxReturned =                    config.maxReturned || 8;

    var txtInput = document.createElement('paper-input');
    txtInput.type ='text';
    ...
    txtHint.autofocus = false;
    txtHint.classList.add('completelytxt')
    
    txtInput.style.backgroundColor ='transparent';
    txtInput.style.verticalAlign = 'top';
    txtInput.style.position = 'relative';
    txtInput.classList.add('completelytxt')
    ...
    wrapper.classList.add('completelywrapper')
    ...
    dropDown.classList.add('completelydd');
        ...
        var p = {
            ...
            refresh : function(token, array) {
                ...
                for (var i=0;i<array.length;i++) {
                    /* determine if the hint matches */
                    var needle = token.toLowerCase().split(" ");
                    if(array[i] == null) continue;
                    var haystack = array[i].toLowerCase();
                    var foundit = true;
                    for(var f = 0; f < needle.length; f++) {
                        if(haystack.indexOf(needle[f]) < 0)
                            foundit = false; 
                    }
                    if(!foundit) { continue; }
                    found++;
                    ...
                    divRow.innerHTML = array[i];
                    rows.push(divRow);
                    elem.appendChild(divRow);
                    if(found >= config.maxReturned) break;
                }
                if (rows.length===0) {
